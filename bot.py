"""
Lannah Meta Bot
- Receives a video from Telegram
- Strips all metadata using FFmpeg (no re-encoding, 4K quality preserved)
- Sends back the clean file as a document
"""
...
# Token comes ONLY from environment variable — never hardcoded
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Hey! Send me any video and I'll strip all the metadata "
        "so Instagram can't detect it's been posted before.\n\n"
        "4K quality stays fully intact — just drop a video."
    )
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if file_size > 2_000_000_000:
        await msg.reply_text("File too large (max 2 GB).")
        return
    ...
    try:
        ...
        await status.edit_text("Stripping metadata...")
        strip_metadata(in_path, out_path)

        await status.edit_text("Sending clean video...")
        ...
        with open(out_path, "rb") as f:
            await msg.reply_document(
                ...
                filename=out_name,
                caption=(
                    "Done. All metadata stripped:\n"
                    "- GPS & location removed\n"
                    "- Device & camera info wiped\n"
                    "- Timestamps cleared\n"
                    "- Encoder fingerprint removed\n\n"
...
def main():
    ...
    log.info("Lannah Meta Bot running...")
